...
from graphics.imports import CLASSES_DICT, ALL_SCENE_NAMES
...
COMBINED_SCENES.append({
    'domfountain_station1_xyz_intensity_rgb': (0, [0,0,0]), 
    'domfountain_station2_xyz_intensity_rgb': (150, [2,-16,0]), 
    'domfountain_station3_xyz_intensity_rgb': (202, [-10,1,0]), 
})
...
COMBINED_SCENES.append({ 'sg28_station4_intensity_rgb': (0, [0,0,0]), })
COMBINED_SCENES.append({
    'untermaederbrunnen_station1_xyz_intensity_rgb': (0, [0,0,0]), 
    'untermaederbrunnen_station3_xyz_intensity_rgb': (-128, [-6,-3,0]), 
})
...
def view_separate(xyz0, rgb0, xyz1, rgb1):
    v=pptk.viewer(xyz0)
    v.attributes(rgb0/255.0)
    v.set(point_size=0.04)
    
    v=pptk.viewer(xyz1)
    v.attributes(rgb1/255.0)
    v.set(point_size=0.04)    
...
if __name__ == "__main__":
    ...
    if sys.argv[1]=='align':
        ...
        view_separate(xyz0, rgb0, xyz1, rgb1)
    ...
    if sys.argv[1]=='combine':
        ...
        for combine_dict in (COMBINED_SCENES[1],):
            save_name=sorted(list(combine_dict.keys()))[0]
            
            if len(combine_dict.keys())==1: #Only Copy
                print('Copy',save_name)
                copyfile( os.path.join(base_in,save_name+'.xyz.npy'), os.path.join(base_out,save_name+'.xyz.npy') )
