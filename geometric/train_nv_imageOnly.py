...
'''
Ablation study for Visual Graph Embedding (NetVLAD): train only on images (using model.encode_images()) to evaluate on S3D and Cambridge
...
print(f'VGE-NV-ImageOnly training: image limit: {IMAGE_LIMIT} bs: {BATCH_SIZE} lr gamma: {LR_GAMMA} embed-dim: {EMBED_DIM} shuffle: {SHUFFLE} margin: {MARGIN} lr:{LR}')
...
for lr in (LR,):
    ...
    for epoch in range(10):
        ...
        for i_batch, batch in enumerate(data_loader):
            ...
            out_visual=model.encode_images(batch['images'].cuda())

            loss=criterion(out_visual, out_visual)
            loss.backward()
            optimizer.step()
...
print('\n----')           
model_name=f'model_VGE-NV-ImageOnly_l{IMAGE_LIMIT}_b{BATCH_SIZE}_g{LR_GAMMA:0.2f}_e{EMBED_DIM}_s{SHUFFLE}_m{MARGIN}_lr{LR}.pth'
...
plt.legend()
#plt.show()
plt.savefig(f'loss_VGE-NV-ImageOnly_l{IMAGE_LIMIT}_b{BATCH_SIZE}_g{LR_GAMMA:0.2f}_e{EMBED_DIM}_s{SHUFFLE}_m{MARGIN}_lr{LR}.png')    
