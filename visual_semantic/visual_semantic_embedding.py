...
class HardestRankingLoss2(torch.nn.Module):
    def __init__(self, margin=1.0):
        super(HardestRankingLoss2, self).__init__()
        self.margin = margin
    ...
    def forward(self, im, s): #Norming the input (as in paper) is actually not helpful
        ...
        for i in range(scores.size()[0]):
            cost_s[i, i] = 0
            rowmax=torch.max(cost_s[i,:])
            for j in range(scores.size()[0]):
                if cost_s[i,j]<rowmax:
                    cost_s[i,j]=0

            cost_im[i, i] = 0
            rowmax=torch.max(cost_im[i,:])
            for j in range(scores.size()[0]):
                if cost_im[i,j]<rowmax:
                    cost_im[i,j]=0            
