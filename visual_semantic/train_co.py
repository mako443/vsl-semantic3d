...
from .visual_semantic_embedding import VisualSemanticEmbeddingCombined
...
print(f'VSE-CO training: image limit: {IMAGE_LIMIT} bs: {BATCH_SIZE} lr gamma: {LR_GAMMA} embed-dim: {EMBED_DIM} shuffle: {SHUFFLE} margin: {MARGIN} lr: {LR}')
...
#for lr in (2.5e-3,1e-3,7.5e-4):
for lr in (LR,):
    print('\n\nlr: ',lr)

    vgg=create_image_model_vgg11()
    model=VisualSemanticEmbeddingCombined(vgg,data_set.get_known_words(), EMBED_DIM).cuda()
    ...
    for epoch in range(10):
        ...
        for i_batch, batch in enumerate(data_loader):
            ...
            a_out=model(batch['images_anchor'].to('cuda'),   batch['captions_anchor'])
            p_out=model(batch['images_positive'].to('cuda'), batch['captions_positive'])
            n_out=model(batch['images_negative'].to('cuda'), batch['captions_negative'])
...
print('\n----')           
model_name=f'model_VSE-CO_l{IMAGE_LIMIT}_b{BATCH_SIZE}_g{LR_GAMMA:0.2f}_e{EMBED_DIM}_s{SHUFFLE}_m{MARGIN}_lr{LR}.pth'
...
plt.legend()
#plt.show()
plt.savefig(f'loss_VSE-CO_l{IMAGE_LIMIT}_b{BATCH_SIZE}_g{LR_GAMMA:0.2f}_e{EMBED_DIM}_s{SHUFFLE}_m{MARGIN}_lr{LR}.png')    
