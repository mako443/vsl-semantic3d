...
def eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, top_k=(1,3,5,10), thresholds=[(15.0,45), (25.0,60), (50.0,90)], reduce_indices=None):
    ...
    for k in top_k:
        for t in thresholds:
            #Number of threshold hits over the number of considered retrievals (=k)
            thresh_hits[t][k]= np.sum(thresh_hits[t][k]) / np.sum(scene_counts[k])
        # Number of scene hits over the number of considered retrievals (=k)
        scene_hits[k]= np.sum(scene_hits[k]) / np.sum(scene_counts[k])

    return thresh_hits, scene_hits

def eval_netvlad_retrieval_old(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, top_k=(1,3,5,10), reduce_indices=None):
    assert reduce_indices in (None,'scene-voting','scene-voting-double-k')
    print(f'eval_netvlad_retrieval(): # training: {len(dataset_train)}, # test: {len(dataset_test)}')
    print('Reduce indices:',reduce_indices)
...
if __name__ == "__main__":
    ...
    if 'netvlad' in sys.argv:
        ...
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, reduce_indices=None)
        print_topK(thresh_results, scene_results)     
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, reduce_indices='scene-voting')
        print_topK(thresh_results, scene_results)
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, reduce_indices='scene-voting-double-k')
        print_topK(thresh_results, scene_results)                              

        features_name='features_netvlad_Occ-Occ_m0.5_o0.3.pkl'
        netvlad_vectors_train,netvlad_vectors_test=pickle.load(open('evaluation_res/'+features_name,'rb')); print(f'Using features {features_name}')
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test,top_k=(1,3,5,10), reduce_indices=None)
        print_topK(thresh_results, scene_results)  
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test,top_k=(10,15,20), reduce_indices=None)
        print_topK(thresh_results, scene_results) 

        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test,top_k=(1,3,5,10), reduce_indices='scene-voting')
        print_topK(thresh_results, scene_results) 
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test,top_k=(10,15,20), reduce_indices='scene-voting')
        print_topK(thresh_results, scene_results)         

    if 'netvlad-pitts' in sys.argv:
        features_name='features_netvlad_Pitts.pkl'
        netvlad_vectors_train,netvlad_vectors_test=pickle.load(open('evaluation_res/'+features_name,'rb')); print(f'Using features {features_name}')
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, reduce_indices=None)
        print_topK(thresh_results, scene_results)    

        print('\n OLD:')
        pos_results, ori_results, scene_results=eval_netvlad_retrieval_old(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, top_k=(1,3,5,10), reduce_indices=None)
        print(pos_results, ori_results, scene_results)                                           
                             
    if 'gather-cambridge' in sys.argv:
        IMAGE_LIMIT=3000
        BATCH_SIZE=6
        NUM_CLUSTERS=8
        ALPHA=10.0
        ...
        dataset_train=CambridgeDataset('data_cambridge','train',transform=transform)
        dataset_test =CambridgeDataset('data_cambridge','test', transform=transform)
        ...
        gather_netvlad_vectors(dataloader_train, dataloader_test, model, "NV-S3D_cambridge")

    if 'netvlad-cambridge' in sys.argv:
        dataset_train=CambridgeDataset('data_cambridge','train',transform=transform)
        dataset_test =CambridgeDataset('data_cambridge','test', transform=transform)

        features_name='features_netvlad_NV-S3D_cambridge.pkl'
        netvlad_vectors_train,netvlad_vectors_test=pickle.load(open('evaluation_res/'+features_name,'rb')); print(f'Using features {features_name}')
        thresh_results, scene_results=eval_netvlad_retrieval(dataset_train, dataset_test, netvlad_vectors_train, netvlad_vectors_test, reduce_indices=None)
        print_topK(thresh_results, scene_results) 
