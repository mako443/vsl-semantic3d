...
'''
def netvlad_scenegraphs2viewobjects(data_loader_train, data_loader_test,model, top_k=(1,3,5,10)):
    CHECK_COUNT=10
    print(f'# training: {len(data_loader_train.dataset)}, # test: {len(data_loader_test.dataset)}')
    ...
    for idx in check_indices:
        ...
        #Combine NetVLAD (low<->close) & SG scores (high<->close)
    ...
    print('Saving retrieval results...')
    pickle.dump(retrieval_dict, open('retrievals_NetVLAD_SceneGraph2ViewObjects.pkl','wb'))
...
if __name__ == "__main__":
    ...
    if "netvlad+sg" in sys.argv:
        ...
        pos_results, ori_results, scene_results=netvlad_scenegraphs2viewobjects(data_loader_train, data_loader_test, model)
        print(pos_results, ori_results, scene_results)
