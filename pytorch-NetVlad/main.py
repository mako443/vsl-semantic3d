...
#python3 main.py --mode=test --split=val --resume=vgg16_netvlad_checkpoint/
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i_batch, batch in enumerate(data_loader_test):
            a=batch
            image_encoding = model.encoder(a.cuda())
            vlad_encoding = model.pool(image_encoding) 
            netvlad_vectors_test=torch.cat((netvlad_vectors_test,vlad_encoding))
        for i_batch, batch in enumerate(data_loader_train):
            a=batch
            image_encoding = model.encoder(a.cuda())
            vlad_encoding = model.pool(image_encoding) 
            netvlad_vectors_train=torch.cat((netvlad_vectors_train,vlad_encoding))        
